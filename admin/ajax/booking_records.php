...
if(isset($_POST['booking'])){
    ...
    $sql = mysqli_query($conn, "SELECT * FROM `booking_order` 
                                ...
                                ORDER BY booking_order.booking_id DESC");
    $i = 1;
    $table_data = "";
    $status = "";
    while($data = mysqli_fetch_assoc($sql)){
        ...
        if($data['booking_status'] == 'active' && $data['arrival'] == 0){
            $status     = "<span class='badge bg-warning'>ACTIVED BUT NOT ARRIVED</span>";
        }
        elseif($data['booking_status'] == 'active' && $data['arrival'] == 1){
            $status     = "<span class='badge bg-success'>ACTIVED AND ARRIVED</span>";
        }
        elseif($data['booking_status'] == 'cancelled' && $data['refund'] == 0){
            $status     = "<span class='badge bg-danger'>CANCELLED BUT NOT REFUNDED</span>";
        }
        elseif($data['booking_status'] == 'cancelled' && $data['refund'] == 1){
            $status     = "<span class='badge bg-success'>CANCELLED AND REFUNDED</span>";
        ...
        $table_data .= "
            <tr> 
                ...
                <td>
                     <button type='button' onclick='pdf_booking($data[booking_id])' class='btn btn-danger'>
...
if(isset($_POST['assign_room'])){
    $sql = mysqli_query($conn, "UPDATE `booking_order` bo INNER JOIN `booking_details` bd 
                                ...
                                SET bo.arrival = 1, bd.room_no = $_POST[room_number]
