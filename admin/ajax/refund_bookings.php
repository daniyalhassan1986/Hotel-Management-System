...
if(isset($_POST['booking'])){
    ...
    $sql = mysqli_query($conn, "SELECT * FROM `booking_order` 
                                ...
                                WHERE booking_order.refund = 0 AND booking_order.arrival = 0 
    ...
    while($data = mysqli_fetch_assoc($sql)){
        ...
        $table_data .= "
            <tr> 
                ...
                <td>
                    <b>Pkr $data[trans_amount]</b>
                </td>
                <td>
                    <button type='button' onclick='refund($data[booking_id])' class='btn btn-success'>
                        <i class='bi bi-cash'></i> Cancel Booking 
                    </button>
                <td>
            </tr>
...
if(isset($_POST['refund_booking'])){
    $sql = mysqli_query($conn, "UPDATE `booking_order` SET refund = 1
                                WHERE booking_id = $_POST[booking_id]");
    if($sql){
        echo alert('Refund Done', 'success');
    }else{
